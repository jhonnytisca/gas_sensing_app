...
class Dashboard(QMainWindow):
    def __init__(self, logging_config):
        ...
        self.mfc1_val = QSpinBox()
        self.mfc1_val.setRange(0, 2000)
        grid.addWidget(self.mfc1_val, 0, 1)
        grid.addWidget(QLabel("MFC 2 (sccm):"), 1, 0)
        self.mfc2_val = QSpinBox()
        self.mfc2_val.setRange(0, 2000)
        grid.addWidget(self.mfc2_val, 1, 1)
        grid.addWidget(QLabel("MFC 3 (sccm):"), 2, 0)
        self.mfc3_val = QSpinBox()
        self.mfc3_val.setRange(0, 500)
        grid.addWidget(self.mfc3_val, 2, 1)
        grid.addWidget(QLabel("MFC 4 (sccm):"), 3, 0)
        self.mfc4_val = QSpinBox()
        self.mfc4_val.setRange(0, 500)
        grid.addWidget(self.mfc4_val, 3, 1)
        ...
        self.mfc_plot.getAxis("right").setLabel(
            "Shutter State", units="1=Open, 0=Closed"
        )
        self.shutter_view.setYRange(
            -0.1, 1.1, padding=0
        )  # Keeps the binary square waves clean

        self.shutter_curve = pg.PlotCurveItem(
            pen=pg.mkPen(
                color=(142, 68, 173, 200), width=1.5, style=Qt.PenStyle.DashLine
            ),
            fillLevel=0.0,
            brush=pg.mkBrush(142, 68, 173, 35),
        )
        ...
        self.mfc_curves = [
            self.mfc_plot.plot(pen=pg.mkPen(color=c, width=1.8)) for c in mfc_colors
        ]
        ...
        items_to_toggle = [
            ("MFC 1", mfc_colors[0], self.mfc_curves[0]),
            ("MFC 2", mfc_colors[1], self.mfc_curves[1]),
            ("MFC 3", mfc_colors[2], self.mfc_curves[2]),
            ("MFC 4", mfc_colors[3], self.mfc_curves[3]),
        ...
        for name, color, plot_item in items_to_toggle:
            ...
            cb.stateChanged.connect(
                lambda state, item=plot_item: item.setVisible(bool(state))
            )
    ...
    def sync_secondary_axis_views(self):
        ...
        self.shutter_view.linkedViewChanged(
            self.mfc_plot.getViewBox(), pg.ViewBox.XAxis
        )
    ...
    def _setup_print_logging(self):
        try:
            ...
            if console_cfg.get("enabled", False):
                ...
                self.log_file_obj = open(
                    os.path.join(
                        log_dir, f"{prefix}_{time.strftime('%Y%m%d_%H%M%S')}.log"
                    ),
                    "a",
                    encoding="utf-8",
                )
    ...
    def flush_console_queue(self):
        ...
        while not self.console_queue.empty():
            try:
                text_to_append += self.console_queue.get_nowait()
            except queue.Empty:
                break
        if text_to_append:
